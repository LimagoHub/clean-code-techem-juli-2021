...
public abstract class AbstractGame<SCENE,TURN> implements Game {
	...
	private void executeSingleTurn() { // Integration 
		if(initTurnFailed()) return;
		executeConcretePlayersTurn();
		terminateTurn();
	}
	...
	private void executeConcretePlayersTurn() {
	...
	private void checkLosing() {// Operation
		if(isGameOver()) print(String.format(LOSING_MESSAGE, currentPlayer.getName()));
